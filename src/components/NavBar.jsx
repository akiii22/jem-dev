...
function NavBar() {
  ...
  return (
    <nav className="fixed top-0 z-10 flex w-full items-center justify-between border-b border-b-slate-300 bg-gray/70 px-16 py-6 text-white backdrop-blur-md md:justify-evenly">
      <a
        href="#home"
        className="bg-gradient-to-r from-slate-50 to-slate-500 bg-clip-text text-transparent opacity-80 text-3xl font-semibold transition-all duration-300 hover:opacity-100"
      >
        Jerome
      </a>
      ...
      {toggle && (
        <div
          className={`fixed right-0 top-[84px] flex h-screen w-1/2 flex-col items-center justify-evenly gap-10 border-l border-slate-100 bg-black/20 p-12 ${
            toggle ? "block" : "hidden"
          }`}
        >
